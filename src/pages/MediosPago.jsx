...
export default function MediosPago({ isConfigView = false }) {
    ...
    const fetchData = async (searchQuery = '') => {
        setLoading(true);
        try {
            const url = searchQuery ? `/api/v1/medios-de-pago/?search=${searchQuery}` : '/api/v1/medios-de-pago/';
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                await api.put(`/api/v1/medios-de-pago/${editingItem.id}/`, formData);
                toast.success('Medio de pago actualizado');
            } else {
                await api.post('/api/v1/medios-de-pago/', formData);
            ...
            const errStr = error.response?.data?.nombre ? error.response.data.nombre.join(', ') : 'Error al guardar el medio de pago';
            toast.error(errStr);
        }
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api.delete(`/api/v1/medios-de-pago/${deletingItem.id}/`);
            toast.success('Medio de pago eliminado');
            setIsDeleteModalOpen(false);
            fetchData(searchTerm);
        } catch (error) {
            console.error('Error al eliminar:', error);
            toast.error('Error al eliminar, es posible que el medio de pago esté en uso en alguna transacción');
        }
    };
        ...
        <div className={`text-white space-y-6 ${!isConfigView ? 'animate-in fade-in duration-500' : ''}`}>
            {!isConfigView && (
                <div className="flex justify-between items-start">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight mb-2">Métodos de Pago</h1>
                        <p className="text-[#a1a1aa]">Administra las distintas formas en que los clientes y tú pueden realizar transacciones.</p>
                    </div>
            ...
            <div className="bg-[var(--color-bg-card)] border border-[var(--color-border-subtle)] rounded-xl overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-8 text-center text-[#a1a1aa]">No hay métodos de pago registrados.</div>
                ) : (
            ...
            <Modal
                ...
                title={editingItem ? "Editar Método de Pago" : "Nuevo Método de Pago"}
            >
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej. Efectivo, Nequi, Tarjeta de Crédito..."
                        />
                    </div>
                    <div>
                    ...
                    <div className="pt-4 flex justify-end gap-3 border-t border-[#27272a]">
                        ...
                        >
                            {editingItem ? 'Guardar Cambios' : 'Crear Método'}
                        </button>
                    </div>
                </form>
            </Modal>
            ...
            >
                <div className="space-y-4">
                    <p className="text-[#a1a1aa]">
                        ¿Estás seguro de que deseas eliminar el método de pago <strong className="text-white">{deletingItem?.nombre}</strong>?
                        Si existen transacciones hechas con este método, no se podrá borrar por consistencia del sistema.
                    </p>
                    <div className="pt-4 flex justify-end gap-3 border-t border-[#27272a]">
                        <button
                            type="button"
